  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>
          Based on student difficulty feedback submissions
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {topics.length === 0 ? (
          <div className="text-center py-8">
            <IconChartBar className="w-12 h-12 mx-auto text-gray-300 mb-3" />
            <p className="text-gray-500 text-sm">No difficulty feedback data yet</p>
            <p className="text-gray-400 text-xs mt-1">Topics will appear here as students submit difficulty feedback</p>
          </div>
        ) : (
          <>
              ...
              {highDifficultyTopics.map((topic, index) => (
                <div key={index} className="flex items-center justify-between p-2 bg-red-50 rounded-lg">
                  ...
                  <div className="text-xs text-red-600 font-semibold">
                    {topic.studentsStruggling} {topic.studentsStruggling === 1 ? 'request' : 'requests'}
                  </div>
                </div>
              ))}
            </div>
